...
| MR-K | Public deploy | **Chờ người dùng có mặt** | ⏳ Tiếp tục sau MR-U — đã chuẩn bị xong Postgres + Vercel env vars; **mới phát hiện cần đổi rate-limit sang Postgres trước khi deploy serverless** (xem MR-U) |
...
**Cố tình chưa sửa (đã đánh giá, không phải bỏ sót)** — xem thêm mục 4 `PROJECT_STATUS.md`:
- **Rate-limit in-memory** (`lib/login-rate-limit.ts`, `lib/ai/rate-limit.ts`): đúng như comment sẵn trong code, chỉ là vấn đề thật khi chạy nhiều instance/serverless — hiện tại 1 instance duy nhất nên chưa có rủi ro. Phải sửa (chuyển sang lưu ở Postgres, không cần thêm dịch vụ mới vì DB đã có sẵn) **trước khi** deploy Vercel (MR-K), không phải bây giờ.
- **`Payment.status` là String thay vì enum, `choices`/`statements`/`answers` là JSON string thay vì bảng riêng**: đúng là thiết kế chưa chặt, nhưng sửa là refactor lớn ảnh hưởng nhiều file, rủi ro cao hơn lợi ích trước mắt với <10 người dùng hiện tại.
- **26 nhóm câu hỏi trùng nội dung+đáp án** (56/1504 câu, 3,7%) giữa các đề "dự đoán TN THPT 2026" khác nguồn — kiểm tra bằng cách so khớp `content+choices` (loại bỏ được nhiều "trùng giả" chỉ trùng câu dẫn chung chung như "Phát biểu nào sau đây đúng?" mà đáp án khác nhau). Có thể là trùng lặp tự nhiên giữa các đề mock sưu tầm từ nhiều nguồn (hay dùng chung 1 số câu kinh điển) — chưa rõ nên xoá bớt hay giữ, để người dùng quyết định.
- CI (lint/test/build tự động), error boundary/structured logging, dashboard giáo viên xem học sinh yếu ở đâu, spaced repetition cho flashcard, mobile bottom-nav — đều là tính năng/hạ tầng lớn, cần xác nhận độ ưu tiên trước khi bắt tay làm thay vì tự quyết làm hết trong 1 phiên.
