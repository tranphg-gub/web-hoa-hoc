...
_Cập nhật: 2026-07-11 (sau khi hoàn thành GD1-GD3, MR-A→MR-M, trừ MR-K, MR-N mining nội dung thật cho Lớp 11, MR-O mở rộng xuống Lớp 6-7, MR-P mining thêm câu hỏi Lớp 8-9, MR-Q khảo sát kho "HSG chuyên đề", MR-R nâng cấp sơ đồ tư duy + cấu trúc đề mới CT GDPT 2018 + hình ảnh/slide tài liệu, và MR-S — hoàn thiện hạ tầng AI soạn câu hỏi hàng loạt, mining thêm ~640 câu, và sửa 1 loạt lỗi bảo mật/toàn vẹn dữ liệu theo báo cáo review). File này là "bức tranh toàn cảnh" — đọc file này trước để nắm thực trạng, thay vì đọc lại toàn bộ code._
...
- **Các phát hiện từ báo cáo review đã đánh giá nhưng cố tình CHƯA sửa** (không phải bỏ sót): rate-limit in-memory (xem trên); `Payment.status` và các trường `choices`/`statements`/`answers` đang lưu String/JSON thay vì enum/bảng riêng — sửa được nhưng là refactor lớn, rủi ro cao hơn lợi ích trước mắt với quy mô <10 người dùng hiện tại; 26 nhóm câu hỏi trùng nội dung+đáp án giữa các đề "dự đoán TN THPT 2026" khác nhau (56/1504 câu, 3,7%) — có thể là trùng lặp tự nhiên giữa các đề thi thử sưu tầm từ nhiều nguồn (các đề mock thường dùng chung 1 số câu kinh điển), chưa rõ nên xoá bớt hay giữ nguyên; CI (lint/test/build tự động), dashboard giáo viên, spaced repetition, mobile bottom-nav — đều là tính năng lớn cần người dùng xác nhận độ ưu tiên trước khi làm.
...
- Có mặt khi triển khai public (theo đúng yêu cầu của bạn) — chọn nền tảng hosting (Vercel là lựa chọn tự nhiên nhất cho Next.js). **Nhắc: cần đổi rate-limit sang Postgres trước khi deploy (xem mục 4).**
...
Tổng: **1.504 câu hỏi đề kiểm tra** (1.494 SINGLE_CHOICE, 4 TRUE_FALSE_GROUP, 6 SHORT_ANSWER — hai dạng mới vẫn chiếm tỉ lệ rất nhỏ, chủ yếu từ vài đề "dự đoán TN THPT 2026" của Lớp 12) + **204 câu `/practice`**.
